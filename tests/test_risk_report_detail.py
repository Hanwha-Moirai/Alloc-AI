...
def _insert_risk_analysis(project_id: int, summary: str, rationale: str) -> int:
    host = os.getenv("RAG_MARIADB_HOST", "localhost")
    port = int(os.getenv("RAG_MARIADB_PORT", "3306"))
    user = os.getenv("RAG_MARIADB_USER")
    password = os.getenv("RAG_MARIADB_PASSWORD")
...
_load_seed_sql()
report_id = _insert_risk_analysis(1, "요약 테스트 상세", "근거 테스트 상세")

url = f"http://localhost:8000/api/projects/1/docs/risk_reports/{report_id}"
res = requests.get(url)

print("=" * 100)
print("[TEST] Risk report detail")
...
if res.status_code == 200:
    ...
    print(res.text)
    raise RuntimeError(f"Detail request failed with status {res.status_code}")
